...
## Автоматическое обнаружение gateway

Если текущий gateway перестал отвечать, monitor проверяет тот же reverse-порт на
всех gateway, включая central. На найденном соединении выполняется только чтение
hostname роутера. Привязка меняется автоматически только при единственном
совпадении с ожидаемым идентификатором роутера, после чего новый gateway
сохраняется в PostgreSQL и используется после перезапуска monitor.

Кнопка «Проверить сейчас» запускает принудительный поиск по всем gateway даже
при рабочем текущем соединении. Если найдено ноль или несколько совпадений,
привязка не изменяется и создаётся обычный результат проверки с ошибкой.
